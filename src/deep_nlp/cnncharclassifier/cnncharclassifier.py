...
class CNNCharClassifier(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.feature_num= self.params["feature_num"] # vocab size
        self.feature_size= self.params["feature_size"]
        self.kernel_one= self.params["kernel_one"] # 7
        self.kernel_two= self.params["kernel_two"] # 3
        self.stride_one= self.params["stride_one"] # 1
        self.stride_two= self.params["stride_two"] # 3
        self.input_linear= int(((self.seq_len - 96)/27)*self.feature_size)
        self.output_linear= self.params["output_linear"] # 1024
        self.num_class= int(self.params["num_class"]) # 2

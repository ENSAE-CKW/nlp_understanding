...
from deep_nlp.bert_cnn import BERTCNNClassifier
...
def evaluation(model, valid_data
               , bertcnn_size_batch, bertcnn_num_threads, bertcnn_cuda_allow):
    model.eval()

    corrects, avg_loss, epoch_loss, size= 0, 0, 0, 0
    predictions_all, target_all, probabilities_all= [], [], []

    valid_load = DataLoader(valid_data, batch_size= bertcnn_size_batch
                            , num_workers= bertcnn_num_threads
...
def train(train_data, valid_data, bertcnn_freq_verbose: int, bertcnn_clip: int
          , bertcnn_cuda_allow: bool, bertcnn_lr: float, bertcnn_patience: int, bertcnn_num_epochs: int
          , bertcnn_size_batch: int, bertcnn_num_threads: int
          , bertcnn_max_seq_len: int, bertcnn_embedding_dim: int, bertcnn_nb_filter: int
          , bertcnn_type_filter: Iterable[int]
          , bertcnn_output_dim: int, bertcnn_dropout: float
          , bertcnn_bert_path: str
          ):
    # Set up CUDA
    if bertcnn_cuda_allow:
    ...
    kwargs= {"bert": bert_model, "max_sentence_size": bertcnn_max_seq_len
        ...
        , "height_filter": list(bertcnn_type_filter.values())
        , "output_dim": bertcnn_output_dim, "dropout": bertcnn_dropout}

    model= BERTCNNClassifier(**kwargs)
    ...
    optimizer= optim.Adam(model.parameters(), lr= bertcnn_lr)
    ...
    es= EarlyStopping(patience= bertcnn_patience)

    # Start training
    for epoch in range(bertcnn_num_epochs):
        start_time = time.time()

        train_load = DataLoader(train_data, batch_size= bertcnn_size_batch
                                , num_workers= bertcnn_num_threads
                                , drop_last=True, shuffle=True, pin_memory= True)

        model.train()
        train_loss= train_model(model= model, optimizer= optimizer, train_load= train_load, epoch= epoch
                                , cnn_cuda_allow= bertcnn_cuda_allow, cnn_clip= bertcnn_clip
                                , cnn_freq_verbose= bertcnn_freq_verbose, cnn_size_batch= bertcnn_size_batch)


        # Save the model if its the best encountered for so long dude (after 1 entire epoch)
        model.eval()
        valid_results = evaluation(model= model, valid_data= valid_data,  bertcnn_size_batch= bertcnn_size_batch
...
def bert_test(test_data, cnn_cuda_allow: bool, cnn_size_batch: int
